...
class CreateSingupInputSchema(ma.Schema):
...
class CreateLoginInputSchema(ma.Schema):
    # the 'required' argument ensures the field exists
    email = fields.Email(required=True)
    password = fields.Str(required=True, validate=validate.Length(min=6))
...
# The CreateResetPasswordEmailSendInputSchema class is a schema that validates the input for the
# create_reset_password_email_send function
class CreateResetPasswordEmailSendInputSchema(ma.Schema):
    # the 'required' argument ensures the field exists
    email = fields.Email(required=True)

class ResetPasswordInputSchema(ma.Schema):
    # the 'required' argument ensures the field exist
    password = fields.Str(required=True, validate=validate.Length(min=6))