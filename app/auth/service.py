...
def create_user(request, input_data):
    ...
    create_validation_schema = CreateSingupInputSchema()
    errors = create_validation_schema.validate(input_data)
    
    if errors:
        return generate_response(message=errors)
    
    check_username_exist = User.simple_filter(username=input_data.get('username')).first()
    check_email_exist = User.simple_filter(email=input_data.get('email')).first()
    
    if check_username_exist:
        return generate_response(message="Username already exist", status=HTTP_400_BAD_REQUEST)
    elif check_email_exist:
        return generate_response(message="Email already taken", status=HTTP_400_BAD_REQUEST)
    
    new_user = User(**input_data) # Create an instance of the User class
    new_user.hash_password()
    new_user.save() # Adds new User record to database
    
    del input_data['password']
    
    return generate_response(data=input_data, message="User Created", status=HTTP_201_CREATED)
...
def login_user(request, input_data):
    ...
    get_user = User.simple_filter(email=input_data.get('email')).first()
    ...
    if get_user.check_password(input_data.get('password')):
        ...
        return generate_response(data=input_data, message="User login successfully", status=HTTP_200_OK)
    else:
        return generate_response(message="Password is wrong", status=HTTP_400_BAD_REQUEST)
...
def reset_password_email_send(request, input_data):
    ...
    user = User.simple_filter(email=input_data.get('email')).first()
    
    if user is None:
        return generate_response(message="No record found with this email. Please singup first", status=HTTP_400_BAD_REQUEST)
    
    send_forgot_password_email(request, user)
    
    return generate_response(message="Link sent to the registered email address", status=HTTP_200_OK)
